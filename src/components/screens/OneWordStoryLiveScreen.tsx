...
/**
 * Live-type one-word story. Presenter types each word as the audience shouts it.
 * Enter locks the word, Backspace on empty input undoes the last word, Reset clears.
 */
export const OneWordStoryLiveScreen = () => {
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center px-12 animate-fade-in">
      <div className="max-w-7xl w-full space-y-8">
        ...
        <div
          className="min-h-[420px] bg-card/60 border-2 border-border rounded-3xl p-12 shadow-xl flex flex-wrap items-center justify-center gap-x-5 gap-y-4"
          aria-live="polite"
        >
          {words.length === 0 && !draft && (
            <p className="text-3xl text-muted-foreground/50 italic">…</p>
          )}
          {words.map((w, i) => (
            <span
              key={i}
              className="text-6xl md:text-7xl font-display text-foreground animate-fade-in"
            >
              {w}
            </span>
          ))}
          {draft && (
            <span className="text-6xl md:text-7xl font-display text-primary/60 italic">
              {draft}
            </span>
          )}
        </div>
